...
object OperationsDuringCharge extends App {
  ...
  val DATDF = spark.read.option("mergeSchema","true").parquet(userEventPath).select("id","event","event_date").sort($"event_date")
  ...
  val appUseLiteracyDF = spark.read.option("mergeSchema","true").parquet(appUseLiteracyLogs).select("imei","package_name","event_date")
  ...
  batteryChargeDuration.coalesce(1).write.mode(saveMode = "overwrite").option("header","true").csv(userEventsStatsOutputPath)
