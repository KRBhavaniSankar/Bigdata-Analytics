...
object UserEventLogsEDA extends App {
  ...
  val validRecord = sampleUserDF.select("id","model","event","event_date")
    .filter($"event_date".between("2017-03-29 00:00:00","2018-10-31 23:59:59"))
    .sort($"event_date")

  //validRecord.show(50,false)
  val windowOverCond= Window.partitionBy($"id",$"model").orderBy($"event_date")
  ...
  val batteryChargeStats = batteryChargeDuration.filter($"charge_duration" >= 120)
                                                ...
                                                ,avg($"charge_duration").alias("battery_charge_avg_time").cast("integer"))
